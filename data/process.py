...
def extract_face_embeddings_from_image(app: FaceAnalysis, img: np.ndarray) -> Tuple[List, List, List]:
    # get the faces
    faces = app.get(img)
    # extract the face embeddings
    embeddings = [f.embedding for f in faces]
    # extract bboxs
    bboxs = [f.bbox for f in faces]
    # return the face embeddings
    return faces, embeddings, bboxs
...
def extract_face_embeddings_from_folder(folder: str, out_folder: str, draw: bool = False) -> None:
    ...
    for _, image in enumerate(images):
        ...
        # extract the face embeddings
        res, embeddings, bboxs = extract_face_embeddings_from_image(app, _img)
        # save bbox and embeddings to a numpy file
        np.save(os.path.join(out_folder, image) + '.npy', {"bbox": bboxs, "embeddings": embeddings})
